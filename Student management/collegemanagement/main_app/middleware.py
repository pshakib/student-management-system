...
class LoginCheckMiddleWare(MiddlewareMixin):
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if user.is_authenticated:
            ...
            if request.path == reverse('index') or modulename == 'django.contrib.auth.views' or request.path == reverse('doLogin'): # If the path is login or has anything to do with authentication, pass
                pass
            else:
                return redirect(reverse('index'))